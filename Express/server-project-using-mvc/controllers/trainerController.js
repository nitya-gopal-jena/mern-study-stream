...
const createStudent = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'trainer') {
      return res.status(403).json({ message: 'Access denied: Only trainers can create student account' });
    }
    ...
    const newstudentArr = { name, ...req.params, password: hashPassword, age };

    studentArr.push(newstudentArr);
...
module.exports = { trainerSignup, trainerLogin, createStudent };
